...
// 创建菜单
Wechat.prototype.createMenu = (menu) => {
    return new Promise((resolve, reject) => {
        this.fetchAccessToken()
            .then((data) => {
...
// 获取当前菜单配置项
Wechat.prototype.getCurrentMenu = () => {
    return new Promise((resolve, reject) => {
        this.fetchAccessToken()
            .then((data) => {
...
// 创建二维码
Wechat.prototype.createQrcode = (qr) => {
    return new Promise((resolve, reject) => {
        this.fetchAccessToken()
            .then((data) => {
...
// 展示二维码, 返回一张图片链接 ticket 有效期30分钟
Wechat.prototype.showQrcode = (ticket) => {
    return api.qrcode.show + 'ticket=' + encodeURI(ticket);
}

// 创建二维码
Wechat.prototype.createShorturl = (action, url) => {
    action = action || 'long2short';

    return new Promise((resolve, reject) => {
        this.fetchAccessToken()
            .then((data) => {
                var _url = api.qrcode.current + '?access_token=' + data.access_token;
                var form = {
                    action: action,
                    long_url: url
                }
                request({ method: 'POST', url: _url, body: form, json: true })
                    .then((response) => {
                        var data = response[1];
                        data ? resolve(data) : reject(data);
                    });
...
// 语义搜索
Wechat.prototype.semantic = (semanticData) => {
    return new Promise((resolve, reject) => {
        this.fetchAccessToken()
            .then((data) => {
                var _url = api.semanticUrl + '?access_token=' + data.access_token;
                semanticData.appid = data.appID;
                request({ method: 'POST', url: _url, body: semanticData, json: true })
                    .then((response) => {
                        var data = response[1];
                        data ? resolve(data) : reject(data);
                    });
